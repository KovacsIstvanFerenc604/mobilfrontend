...
export default class Gephaz extends Component {
  ...
  async getMovies() {
    try {
      const response = await fetch('http://192.168.6.5:3000/gephaz');
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              ...
              <View style={{marginBottom:30}}>
                ...
                <Image source={{uri:'http://192.168.6.5:3000/'+item.gephaz_kep+'.jpg'}} style={styles.kep}   />
                </View>
              
              <View style={{flex:3}}>
              <Text style={styles.adatok}>{item.gephaz_alaplaptipus}</Text>          
              </View>
              </View>
            )}
          />
